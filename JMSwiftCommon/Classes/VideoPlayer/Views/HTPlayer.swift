...
public class HTPlayer: UIView {
    ...
    public func ht_setSubTitleStatus( _ status:Int) {
        
        subTitleStatus = status
        
        contentView.ht_setSubTitleStatus(status)
    }
    
    public func ht_setSelectEpisodeButtonTitle( _ title:String) {
    ...
    /// 关闭横屏右侧视图
    public func ht_setHiddenFullScreenRightPanelView() {
        
        self.contentView.ht_setHiddenFullScreenRightPanelView()
    }
    
    /// 是否显示顶部广告banner视图
    public func ht_setHiddenFullScreenTopADBannerView( _ hidden:Bool) {
        
        self.contentView.ht_setHiddenFullScreenTopADBannerView(hidden)
    }
    
    /// 是否显示暂停广告视图
    public func ht_setHiddenFullScreenPauseADView( _ hidden:Bool) {
        
        self.contentView.ht_setHiddenFullScreenPauseADView(hidden)
    }
    
    public func ht_setADView() {
        
        ht_setFullScreenPauseADView()
...
//        NotificationCenter.default.addObserver(self, selector: #selector(ht_appDidEnterBackground), name: UIApplication.willResignActiveNotification, object: nil)
...
public extension HTPlayer {
    ...
    func ht_loadSTRFile(fileFromRemote filePath: URL, encoding: String.Encoding = .utf8) {
        ...
        contentView.ht_loadSTRFile(fileFromRemote: filePath,encoding: encoding)
    }
    
    func ht_loadSTRData(fileData data:Data?) {
        
        contentView.ht_loadSTRData(fileData: data)
    }
}
...
extension HTPlayer: HTPlayerContentViewDelegate {
    ...
    func ht_didClickFailButton(in _: HTPlayerBaseContentView) {
        guard let url = url else { return }
        self.url = url
    }
    ...
    func ht_setFullScreenPauseADView () {
    ...
    func ht_setFullScreenTopBannerADView () {
